...
export const StatCard: React.FC<StatCardProps> = ({ title, value, change, isPositive, icon, color }) => {
  ...
  const themeClasses = {
    blue: {
      ...
      text: 'text-blue-400',
      iconBg: 'bg-blue-950/80 text-blue-400 border border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.1)]',
    ...
    red: {
      ...
      text: 'text-red-400',
      iconBg: 'bg-red-950/80 text-red-400 border border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.1)]',
    ...
    green: {
      ...
      iconBg: 'bg-emerald-950/80 text-emerald-400 border border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.1)]',
    ...
    purple: {
      ...
      iconBg: 'bg-purple-950/80 text-purple-400 border border-purple-500/30 shadow-[0_0_15px_rgba(139,92,246,0.1)]',
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-3 text-[7px] font-mono text-slate-600 tracking-widest select-none pointer-events-none opacity-40 group-hover:opacity-80 transition-opacity">
      ...
      <div className="flex justify-between items-start relative z-10">
        <div className="space-y-1">
          <p className="text-slate-500 text-[10px] font-mono uppercase tracking-widest flex items-center gap-1.5">
            <span className={`w-1.5 h-1.5 rounded-full ${color === 'green' ? 'bg-emerald-500' : color === 'red' ? 'bg-red-500' : color === 'purple' ? 'bg-purple-500' : 'bg-blue-500'} animate-ping`}></span>
            {title}
          </p>
        ...
        <div className="relative">
          ...
          <div className={`p-3.5 rounded-lg relative ${config.iconBg} transition-transform duration-300 group-hover:scale-110`}>
            {icon}
          </div>
        </div>
      </div>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-[2px] w-full ${config.line} opacity-40 group-hover:opacity-100 transition-opacity`} />
    </motion.div>
  );
};
