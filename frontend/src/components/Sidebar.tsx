...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const themeColors = {
    blue: {
      ...
      activeBg: 'bg-blue-600/10 text-blue-400 border border-blue-600/20 shadow-[0_0_15px_rgba(59,130,246,0.1)]',
    ...
    green: {
      ...
      activeBg: 'bg-emerald-600/10 text-emerald-400 border border-emerald-600/20 shadow-[0_0_15px_rgba(16,185,129,0.1)]',
    ...
    purple: {
      ...
      activeBg: 'bg-purple-600/10 text-purple-400 border border-purple-600/20 shadow-[0_0_15px_rgba(139,92,246,0.1)]',
  ...
  const menuItems = [
    { id: View.DASHBOARD, label: 'Dashboard', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg> },
    { id: View.NEWS_SCANNER, label: 'Fake News Detector', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg> },
    { id: View.CYBER_MONITOR, label: 'Cyber Threat Monitor', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg> },
    { id: View.EMAIL_SCANNER, label: 'Gmail Intel Scanner', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg> },
    { id: View.PROFILE, label: 'My Profile', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg> },
  ];
    ...
    <div className="w-64 bg-slate-950 border-r border-slate-900/80 flex flex-col h-full fixed left-0 top-0 z-20 shadow-2xl relative overflow-hidden">
      ...
      <div className="p-6 border-b border-slate-900/60 flex items-center justify-between relative z-10">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-base font-extrabold text-white tracking-widest font-mono">CEREBRO</h1>
            <span className="text-[8px] text-slate-500 font-mono tracking-wider uppercase">[ LVL_10_CLEARANCE ]</span>
          </div>
        </div>
      ...
      <nav className="flex-1 p-4 space-y-1.5 relative z-10 overflow-y-auto custom-scrollbar">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`transition-transform duration-300 group-hover:scale-110 ${isActive ? style.text : 'text-slate-500 group-hover:text-slate-300'}`}>
                {item.icon}
              </div>
      ...
      <div className="p-4 border-t border-slate-900/60 bg-slate-950/30 relative z-10 flex flex-col items-center">
        ...
        <div className="relative w-36 h-36 flex items-center justify-center">
          ...
          <div className={`absolute inset-0 border border-dashed rounded-full ${
            systemColor === 'green' ? 'border-emerald-500/30' : systemColor === 'purple' ? 'border-purple-500/30' : 'border-blue-500/30'
          } ${isDialRotating ? 'animate-omnitrix-spin-fast' : 'animate-omnitrix-spin-slow'}`} />

          {/* Secondary rotating notched ring */}
          <div className={`absolute inset-2 border-2 border-dotted rounded-full scale-95 opacity-50 ${
            systemColor === 'green' ? 'border-emerald-500/40' : systemColor === 'purple' ? 'border-purple-500/40' : 'border-blue-500/40'
          ...
          >
            ...
            <div className={`absolute inset-0 opacity-20 pointer-events-none group-hover:opacity-40 transition-opacity bg-radial-lines ${
              isDialRotating ? 'animate-spin [animation-duration:1s]' : 'animate-spin [animation-duration:8s]'
            }`} style={{
              backgroundImage: 'repeating-conic-gradient(from 0deg, #fff 0deg 10deg, transparent 10deg 20deg)'
            }} />

            {/* Pulsing center green core */}
            <div className={`w-3.5 h-3.5 rounded-full absolute ${style.coreGlow} ${isDialRotating ? 'scale-150' : 'animate-pulse'}`} />

            {/* Omnitrix icon wings / overlays */}
            <div className={`absolute w-12 h-1 ${systemColor === 'green' ? 'bg-emerald-500/30' : systemColor === 'purple' ? 'bg-purple-500/30' : 'bg-blue-500/30'} rotate-45 pointer-events-none`} />
            <div className={`absolute w-12 h-1 ${systemColor === 'green' ? 'bg-emerald-500/30' : systemColor === 'purple' ? 'bg-purple-500/30' : 'bg-blue-500/30'} -rotate-45 pointer-events-none`} />
