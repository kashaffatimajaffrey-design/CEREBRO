...
export const Profile: React.FC = () => {
  ...
  const [sortOption, setSortOption] = useState<'NEWEST' | 'OLDEST' | 'RISK_HIGH' | 'RISK_LOW'>(
    'NEWEST',
  );
  ...
  const getHistoryColor = (type: string, result: string) => {
    if (type === 'NEWS') {
      ...
      if (lower.includes('safe') || lower.includes('low'))
              ...
              <div className="space-y-4 max-w-md">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-4 py-2 rounded text-sm font-medium"
                  >
                    Cancel
                  </button>
                </div>
                ...
                <div className="absolute right-0 bottom-full mb-2 w-72 p-4 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-50 text-xs text-slate-300 translate-y-2 group-hover:translate-y-0">
                  ...
                  <div className="space-y-2">
                    <div className="flex gap-2 items-start">
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 uppercase tracking-wider mt-0.5">
                        News
                      </span>
                      <p>
                        Inverted credibility score. <br />
                        <span className="text-slate-500">Example: 10% Credible = 90% Risk</span>
                      </p>
                    </div>
                    <div className="border-t border-slate-800 my-2"></div>
                    <div className="flex gap-2 items-start">
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-400 uppercase tracking-wider mt-0.5">
                        Cyber
                      </span>
                      <p>
                        Mapped from threat level.
                        <br />
                        <span className="text-slate-500">
                          Critical(100) &gt; High(75) &gt; Medium(50) &gt; Low(25) &gt; Safe(0)
                        </span>
                      </p>
                    </div>
                  </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden animate-fade-in shadow-sm">
            <div className="divide-y divide-slate-800/50">
              {filteredAndSortedHistory.map((item) => (
                <div
                  key={item.id}
                  className="p-4 hover:bg-slate-800/40 transition-colors flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center group"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-1.5">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded tracking-wider ${item.type === 'NEWS' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}
                      >
                    ...
                    <p className="text-slate-200 text-sm font-medium truncate group-hover:text-white transition-colors pr-4">
                      {item.summary}
                    </p>
                  </div>
                  <div
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold border whitespace-nowrap shadow-sm ${getHistoryColor(item.type, item.result)}`}
                  >
                    {item.result}
                  </div>
                </div>
