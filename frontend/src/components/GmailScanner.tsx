...
export const GmailScanner: React.FC = () => {
  ...
  const handleOAuthLogin = async () => {
    try {
      ...
      if (err?.code === 'auth/popup-closed-by-user') {
        setError(
          'Google Sign-in popup was closed before completing authentication. Please try again or use manual token fallback.',
        );
        ...
        setError(
          err.message ||
            'Failed to authenticate with Google. You can also paste an OAuth Access Token directly below.',
        );
      }
    } finally {
      setLoading(false);
  ...
  const fetchInbox = async (token: string) => {
    ...
    try {
      ...
      console.error(err);
      setError(
        `Google Workspace Access Denied: ${err.message || 'Check your permissions/token scope.'}`,
      );
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              Search real-time corporate workspace communications, flag spoofing links, phishing
              attempts, and forward fake news.
            </p>
          </div>
            ...
            <div className="space-y-1">
              {error.includes('leaked') || error.includes('403') ? (
                <>
                  ...
                  <span>
                    Your server-side Gemini API key has been reported as leaked by Google. Please
                    generate a new key and update it in the <strong>Settings &gt; Secrets</strong>{' '}
                    panel in AI Studio to restore full email threat assessment.
                  </span>
                </>
              ) : (
                <span>{error}</span>
          ...
          <div className="mt-6 border border-slate-800 bg-slate-950/60 rounded-xl p-8 text-center max-w-2xl mx-auto space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-slate-400">
                To run AI analytics on inbox threats, authorize the CEREBRO workspace to contact
                your Gmail Workspace logs securely.
              </p>
            </div>
            ...
            {isShowingCredentialsHelp && (
              ...
              >
                ...
                <div className="space-y-1 pt-2 border-t border-slate-800">
                  <label className="block text-slate-400 font-medium font-bold text-slate-300">
                    Direct Access Token Override (Best Sandbox Fallback):
                  </label>
                  <textarea
                    rows={3}
                    value={accessToken}
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <p className="text-[10px] text-slate-500">
                      Perfect if third-party popup authorization limits are blocked inside the AI
                      Studio iframe. Get a quick key at Google OAuth Playground.
                    </p>
                    <button
              ...
              >
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 15H15.82a8.001 8.001 0 0113.83-6h.582M4 4a8.001 8.001 0 018-8v8h8"
                  />
                </svg>
              </button>
            </div>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center p-8 text-center h-full text-slate-500">
                  ...
                  <p className="text-xs text-slate-600 mt-1">
                    No corporate emails returned or lack of permission.
                  </p>
                </div>
              ) : (
                emails.map((email) => {
                  ...
                  let badge = (
                    <span className="bg-slate-800 text-slate-400 text-[10px] uppercase font-bold px-1.5 py-0.5 rounded">
                      Unscanned
                    </span>
                  );
                  if (email.status === 'SAFE') {
                    badge = (
                      <span className="bg-emerald-950/60 text-emerald-400 border border-emerald-800/40 text-[10px] uppercase font-bold px-1.5 py-0.5 rounded">
                        Safe
                      </span>
                    );
                  } else if (email.status === 'SUSPICIOUS') {
                    badge = (
                      <span className="bg-amber-950/60 text-amber-400 border border-amber-800/40 text-[10px] uppercase font-bold px-1.5 py-0.5 rounded">
                        Suspicious
                      </span>
                    );
                  } else if (email.status === 'HIGH_RISK') {
                    badge = (
                      <span className="bg-red-950/60 text-red-400 border border-red-800/40 text-[10px] uppercase font-bold px-1.5 py-0.5 rounded">
                        High Risk
                      </span>
                    );
                  }
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start gap-2 mb-1">
                        <span className="text-xs font-bold text-slate-350 truncate max-w-[150px]">
                          {email.from}
                        </span>
                      ...
                      <h5 className="text-sm font-semibold text-white truncate mb-1">
                        {email.subject}
                      </h5>
          ...
          <div className="lg:col-span-7 space-y-6">
            {selectedEmail ? (
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-left space-y-4">
                  <div className="border-b border-slate-800 pb-4 space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-300">
                      <div>
                        <span className="text-slate-550 font-medium">From:</span>{' '}
                        {selectedEmail.from}
                      </div>
                      <div>
                        <span className="text-slate-550 font-medium">Date:</span>{' '}
                        {selectedEmail.date}
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedEmail.forensics ? (
                    <div className="p-5 bg-slate-950 rounded-xl border border-slate-800 space-y-4">
                          ...
                          <div className="relative w-16 h-16 flex items-center justify-center flex-shrink-0">
                            <svg className="w-full h-full rotate-[-95deg]">
                              ...
                              <circle
                                ...
                                r="28"
                                stroke={
                                  selectedEmail.forensics.riskScore > 75
                                    ? '#ef4444'
                                    : selectedEmail.forensics.riskScore > 35
                                      ? '#f59e0b'
                                      : '#10b981'
                                }
                                strokeWidth="4"
                                ...
                                strokeDasharray="175"
                                strokeDashoffset={
                                  175 - (175 * selectedEmail.forensics.riskScore) / 100
                                }
                              />
                            </svg>
                            <span className="absolute text-sm font-bold text-white font-mono">
                              {selectedEmail.forensics.riskScore}%
                            </span>
                          </div>
                          <div>
                            ...
                            <span
                              className={`text-base font-bold ${
                                selectedEmail.forensics.verdict === 'HIGH_RISK'
                                  ? 'text-red-500'
                                  : selectedEmail.forensics.verdict === 'SUSPICIOUS'
                                    ? 'text-amber-500'
                                    : 'text-emerald-500'
                              }`}
                            >
                              {selectedEmail.forensics.verdict === 'HIGH_RISK' &&
                                'CRITICAL PHISHING / SECURITY THREAT'}
                              {selectedEmail.forensics.verdict === 'SUSPICIOUS' &&
                                'SUSPICIOUS LINK / METADATA FLAG'}
                              {selectedEmail.forensics.verdict === 'SAFE' &&
                                'VERIFIED SECURE COMMUNICATIONS'}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-slate-350 leading-relaxed font-sans">
                          {selectedEmail.forensics.reasoning}
                        </p>
                      </div>
                      ...
                      <div className="space-y-2 pt-2 border-t border-slate-800">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedEmail.forensics.indicators.map((ind, i) => (
                            <span
                              key={i}
                              className="bg-slate-900 border border-slate-800 text-slate-300 px-2 py-0.5 rounded text-[10px] font-mono"
                            >
                              ▲ {ind}
                            </span>
                          ))}
                          {selectedEmail.forensics.indicators.length === 0 && (
                            <span className="text-xs text-slate-500">
                              None detected. Header alignment aligns with safe origin.
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="p-3 bg-blue-950/20 border border-blue-900/30 rounded-lg text-xs flex gap-2">
                        ...
                        <div>
                          <strong className="text-blue-400">Security Team Protocol:</strong>{' '}
                          {selectedEmail.forensics.suggestedAction}
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center p-8 bg-slate-950 rounded-xl border border-slate-800/60">
                      <p className="text-sm text-slate-400 mb-4">
                        No risk analysis has been carried out on this message. Execute the forensics
                        engine below.
                      </p>
                      <button
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-base font-bold text-white">
                        Digital Risk Countermeasure Suite
                      </h4>
                    </div>
                    <p className="text-xs text-slate-400">
                      Draft and send quarantine notices or security indicators to original senders
                      or network users utilizing Google Workspace dispatch.
                    </p>
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        {sendLoading
                          ? 'Transmitting Warning Alert...'
                          : 'Dispatch Cyber Advisory Reply'}
                      </button>
                    </div>
                  </motion.div>
                )}
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center flex flex-col items-center justify-center h-[400px] text-slate-500">
                <svg
                  className="w-16 h-16 mb-4 text-slate-700 stroke-1"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <p className="text-xs text-slate-400">
                  Scan details of Workspace traffic nodes for vulnerabilities.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="fixed inset-0 bg-black/80 z-55 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-xs text-slate-300 leading-relaxed">
                You are about to transmit an official cybersecurity advisory email on your personal
                workspace behalf.
              </p>

              <div className="bg-slate-950 p-3 rounded border border-slate-850 space-y-1 text-xs font-mono text-slate-400">
                <div>
                  <span className="text-slate-650">Sender:</span> Your Connected Google Account
                </div>
                <div>
                  <span className="text-slate-650">Recipient:</span> {warningRecipient}
                </div>
                <div>
                  <span className="text-slate-650">Subject:</span> {warningSubject}
                </div>
              </div>
