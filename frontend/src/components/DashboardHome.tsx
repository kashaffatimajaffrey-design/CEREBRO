...
export const DashboardHome: React.FC<DashboardHomeProps> = ({ systemColor = 'blue' }) => {
  ...
  const realFakeNewsCount = history.filter(h => {
    ...
    return h.type === 'NEWS' && (norm.includes('fake') || norm.includes('misleading') || (norm.includes('%') && parseInt(norm) < 50));
  }).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-transparent to-slate-900/30 translate-x-12 -translate-y-12 rounded-full blur-xl`} />
          <h3 className="text-sm font-bold font-mono tracking-widest text-slate-400 mb-6 flex items-center gap-2">
            <Activity className={`w-4 h-4 ${palette.textClass}`} />
            [ THREAT ANALYTICS REAL-TIME OVERVIEW ]
          </h3>
          ...
          <div className="h-80 w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={threatData}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#020617', borderColor: '#1e293b', color: '#f8fafc', borderRadius: 8, fontFamily: 'monospace', fontSize: 11 }}
                />
                <Legend wrapperStyle={{ fontSize: 11, fontFamily: 'monospace', paddingTop: 10 }} />
                <Area type="monotone" dataKey="fakeNews" stroke={palette.primary} fillOpacity={1} fill="url(#colorPrimary)" name="Misinformation Index" strokeWidth={2} />
                <Area type="monotone" dataKey="cyberAttacks" stroke={palette.secondary} fillOpacity={1} fill="url(#colorSecondary)" name="Cyber Penetrations" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        >
          <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-transparent to-slate-900/30 translate-x-12 -translate-y-12 rounded-full blur-xl`} />
          <h3 className="text-sm font-bold font-mono tracking-widest text-slate-400 mb-6 flex items-center gap-2">
            <Globe className={`w-4 h-4 ${palette.textClass}`} />
          ...
          <div className="h-80 w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={threatData}>
                ...
                />
                <Legend wrapperStyle={{ fontSize: 11, fontFamily: 'monospace', paddingTop: 10 }} />
                <Bar dataKey="fakeNews" fill={palette.primary} name="Analyzed Items" radius={[4, 4, 0, 0]} />
                <Bar dataKey="cyberAttacks" fill={palette.accent} name="Threats Mitigated" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
      ...
      <div className={`bg-slate-950/70 border border-slate-900 rounded-xl shadow-2xl overflow-hidden flex flex-col relative ${palette.glow}`}>
        ...
        <div className="p-6 border-b border-slate-900 bg-slate-950/50 flex flex-col xl:flex-row xl:items-center justify-between gap-4 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center bg-slate-950 rounded-lg p-0.5 border border-slate-900 text-[10px] font-mono tracking-wider">
              <button
                onClick={() => { playCyberSFX('hover'); setFilterType('ALL'); }}
              ...
              <button
                onClick={() => { playCyberSFX('hover'); setFilterType('NEWS'); }}
              ...
              <button
                onClick={() => { playCyberSFX('hover'); setFilterType('CYBER'); }}
            ...
            <div className="flex items-center bg-slate-950 rounded-lg p-0.5 border border-slate-900 text-[10px] font-mono tracking-wider">
              <button
                onClick={() => { playCyberSFX('hover'); setFilterRisk('ALL'); }}
              ...
              <button
                onClick={() => { playCyberSFX('hover'); setFilterRisk('HIGH_RISK'); }}
              ...
              <button
                onClick={() => { playCyberSFX('hover'); setFilterRisk('SAFE'); }}
            ...
            {filteredHistory.length === 0 ? (
              ...
              >
                ...
                <div className="space-y-1 font-mono">
                  <h4 className="text-xs font-bold text-slate-400">[ CRYPTOGRAPHIC VAULT STERILE ]</h4>
                  <p className="text-[10px] text-slate-600 max-w-xs mx-auto leading-relaxed">
                    No anomalies found. Run threat intelligence network diagnostics or workspace checks to log forensic traces.
                  </p>
                </div>
              </motion.div>
            ) : (
                  ...
                  >
                    ...
                    <div className="p-4 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className={`p-2 rounded-lg border flex-shrink-0 bg-slate-950 transition-all group-hover:scale-105 duration-300 ${
                          item.type === 'NEWS' ? 'border-blue-500/20 text-blue-400' : 'border-purple-500/20 text-purple-400'
                        }`}>
                          {item.type === 'NEWS' ? (
                        ...
                        <div className="min-w-0 space-y-1">
                          <div className="flex items-center gap-2.5 flex-wrap">
                            <span className={`text-[9px] font-mono font-black tracking-wider bg-slate-950 px-1.5 py-0.5 rounded border border-slate-900 ${
                          ...
                          <div className="flex items-center gap-3 text-[10px] text-slate-500 font-mono">
                            ...
                            <span className="text-slate-600 truncate">REF: {item.id.slice(0, 8).toUpperCase()}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className={`px-2.5 py-1 rounded-full text-[9px] font-mono tracking-wider border font-bold flex items-center gap-1.5 ${style.badgeBg}`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${style.dotBg}`}></span>
                          {item.result.toUpperCase()}
                        </div>

                        {/* Dropdown Chevron Toggle */}
                        <div className="text-slate-600 group-hover:text-slate-400 transition-colors px-1" onClick={() => handleRowClick(item.id)}>
                          ...
                          <div className="px-5 pb-5 pt-1 pl-14 border-t border-slate-900/40 bg-slate-950/40">
                            <div className="bg-slate-950/80 border border-slate-900 p-4 rounded-lg space-y-3 relative overflow-hidden">
                              ...
                              <div className="flex items-center justify-between text-[8px] text-slate-600 font-mono pt-1.5 border-t border-slate-900 relative z-10">
                                <div>
                                  <span className="text-slate-500">INGRESS LINK:</span> users/{user?.id}/history/{item.id}
                                </div>
                                <div>
                                  [ CRYPTO_CLEARED_BY_ALGORITHM ]
                                </div>
                              </div>
                            </div>
                          </div>
